...
export default function Projects() {
  return (
    <section className="max-w-6xl mx-auto px-6 py-12 md:py-20 border-t border-[var(--border)] overflow-hidden">
      ...
      <div className="space-y-24 md:space-y-48">
        {data.projects.map((project, index) => (
          <div key={index} className="group relative">
            <div className="grid-layout items-start transition-all duration-700 hover:md:translate-x-4">
              ...
              <div className="md:col-span-11 flex flex-col gap-6 md:gap-8">
                <div className="space-y-3">
                  ...
                  <h3 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-[var(--text)] leading-tight">
                    {project.title}
                  </h3>
                </div>

                <div className="h-px bg-[var(--border)] w-full group-hover:bg-[var(--accent)]/30 transition-colors duration-500"></div>
            ...
            <div className="absolute top-0 -left-6 md:-left-12 w-px h-0 bg-[var(--accent)] group-hover:h-full transition-all duration-1000 ease-in-out hidden md:block"></div>
          </div>
        ))}
      </div>
    </section>
